...
public class Log {
	...
	public static boolean createFile(String file){
		File f = new File(Bukkit.getPluginManager().getPlugin("MultiTools").getDataFolder(),file+".log");
		...
		try{
			f.createNewFile();
			FileWriter filewriter = new FileWriter(Bukkit.getPluginManager().getPlugin("MultiTools").getDataFolder() + "/" + file + ".log");
			try{
				filewriter.write("");
			}catch (Exception e){
				e.printStackTrace();
